...
def generate_question(topic: str, difficulty: str, context: str | None = None) -> str:
    """Generate an interview question for the given topic and difficulty level.

    Args:
        topic: The technical topic (e.g. 'Python', 'DSA').
        difficulty: Difficulty level ('Easy', 'Medium', 'Hard').
        context: Optional string describing previous questions and weak areas so
                 the model can avoid repetition and probe weak topics deeper.
    """
    if USE_MOCK_LLM:
    ...
    if context:
        template = (
            "You are a technical interviewer conducting a multi-question interview. "
            "Topic: {topic}. Difficulty: {difficulty}. "
            "\n\nContext from this session:\n{context}\n\n"
            "Generate a single NEW interview question that:\n"
            "1. Does NOT repeat any previous question listed above.\n"
            "2. If the candidate scored poorly on any topic mentioned, probes deeper on that topic.\n"
            "3. Progressively increases in specificity compared to earlier questions.\n"
        ...
        try:
            ...
            response = chain.invoke({"topic": topic, "difficulty": difficulty, "context": context})
